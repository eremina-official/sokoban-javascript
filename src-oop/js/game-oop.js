...
class Game {
  ...
  move(direction) {
    ...
    const currentPersonPosition = getPersonPosition(
      this.history[this.history.length - 1]
    );
    const { personY, personX } = currentPersonPosition;
    const { y, x } = direction;
    const nextPersonPosition = {
      nextPersonY: personY + y,
      nextPersonX: personX + x,
    };
    const { nextPersonY, nextPersonX } = nextPersonPosition;
    const nextBoxPosition = {
      nextBoxY: nextPersonY + y,
      nextBoxX: nextPersonX + x,
    };
  ...
  makeMove(
    ...
    this.updateLevelArray(
      levelArray,
      currentPersonPosition,
      nextPersonPosition,
      nextBoxPosition
    );
    this.board.receiveCommand(
      moveType,
      currentPersonPosition,
      nextPersonPosition,
      nextBoxPosition
   ...
   */
  undo(event) {
    if (event.target === this.undoButton && this.history.length > 1) {
      this.history.pop();
