...
class LevelMaker {
  constructor() {
    ...
    this.boardSizeSubmitButtonElement = document.querySelector(
      '.js-board-size-submit'
    );
  ...
  getLevelArray(rowsNumber, columnsNumber) {
    const levelArray = Array(+rowsNumber).fill(
      Array(+columnsNumber).fill('space')
    );
    return levelArray.map((row, rowIndex) =>
  ...
  handleSquareInputChange(event) {
    if (event.target.name !== 'board-square-input') {
      return;
    }
  ...
  handleSquareChange(event) {
    if (!event.target.classList.contains('js-level-maker-square')) {
      return;
    }
  ...
  playCurrentLevelMaker(event) {
    if (!event.target.classList.contains('js-play-current-level-maker')) {
      return;
    }

    if (
      this.playContainerElement.classList.contains('play-level-maker-is-hidden')
    ) {
    ...
    switch (this.checkPerson(currentLevelArray)) {
      case 0:
        this.notificationElement.textContent =
          'Please add a person to the board.';
        break;
      case 1:
        this.notificationElement.textContent = '';
      ...
      default:
        this.notificationElement.textContent =
          'There should be only one person on the board.';
    }
  }
  ...
  getComposedLevelArray() {
    return Array.from(
      this.levelMakerBoardElement.querySelectorAll('.js-board-row')
    ).map((row) =>
      Array.from(row.childNodes).map((square) => square.dataset.square)
    );
  }
